-- PaladinHub V2 point 12: banners/messages. Safe to run repeatedly.
...
-- Soft-deleting a media record referenced by a live banner is refused at database level.
CREATE OR REPLACE FUNCTION ph_protect_banner_media() RETURNS trigger AS $$
BEGIN
    IF NEW."IsDeleted" AND NOT OLD."IsDeleted" AND EXISTS (
...
CREATE TRIGGER "TR_SpellIcons_ProtectBannerMedia"
BEFORE UPDATE OF "IsDeleted" ON "SpellIcons"
FOR EACH ROW EXECUTE FUNCTION ph_protect_banner_media();
