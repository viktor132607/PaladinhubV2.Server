...
public sealed class MediaController : ControllerBase
{
        ...
        CancellationToken ct)
    {
        ...
        return result.Error switch
        {
            ...
            MediaAdminError.Validation => BadRequest(new { message = result.Message }),
            MediaAdminError.NotFound => NotFound(),
            MediaAdminError.Stale => Stale(),
            MediaAdminError.InUse => Conflict(new { message = result.Message }),
            _ => Conflict()
        };
    }
        ...
        CancellationToken ct)
    {
        ...
        {
            MediaAdminError.None => Ok(new { Id = result.Id }),
            MediaAdminError.NotFound or MediaAdminError.RevisionNotFound => NotFound(),
            MediaAdminError.Stale => Stale(),
            MediaAdminError.DeletedRevision => BadRequest(new { message = result.Message }),
            MediaAdminError.InUse => Conflict(new { message = result.Message }),
            _ => Conflict()
        };
    }
    ...
    private IActionResult Stale()
    {
        return Conflict(new
        {
            message = "This image changed in another session. Refresh before saving."
        });
    }
