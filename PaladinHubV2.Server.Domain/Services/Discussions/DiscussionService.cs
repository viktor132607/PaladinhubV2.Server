    ...
    public class DiscussionService : IDiscussionService
    {
        ...
        public async Task<DiscussionPost?> GetByIdAsync(Guid id)
            => await _context.DiscussionPosts
                ...
                .Include(post => post.LikesCollection)
        ...
        {
            var post = await _context.DiscussionPosts.FirstOrDefaultAsync(post => post.Id == id);
            if (post == null)
                return false;
        ...
        public async Task<bool> ToggleCommentLikeAsync(Guid commentId, string userId)
        {
            ...
            var like = await _context.DiscussionCommentLikes
                .FirstOrDefaultAsync(item => item.CommentId == commentId && item.UserId == userId);
        ...
        {
            var postExists = await _context.DiscussionPosts.AnyAsync(post => post.Id == postId);
            if (!postExists)
                return false;
